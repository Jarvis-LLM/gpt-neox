...
def load_checkpoint(
    ...
    if neox_args.deepspeed:
        ...
        if neox_args.initialize_llama_cn_with_llama_word_embeddings:
            custom_load_fn = custom_llama_cn_vocab_expand_load_func
        else:
            custom_load_fn = None
        ...
        ### only enable the training of new added chinese token embeddings and the final linear layer
        if neox_args.only_train_new_chinese:
            for name, param in model.named_parameters():
                if name in ["module.sequential.0.word_embeddings.weight", "module.sequential.36.final_linear.weight"]:
