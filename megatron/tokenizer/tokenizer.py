...
def build_tokenizer(args):
    ...
    elif args.tokenizer_type.lower() == "LlamaTokenizer".lower():
        assert args.vocab_file is not None
        tokenizer =  _LlamaTokenizer(args.vocab_file)
    else:
...
class _LlamaTokenizer(AbstractTokenizer):
    ...
    def __init__(self, vocab_file):
        name = "_LlamaTokenizer"
        super().__init__(name)

        self.tokenizer = LlamaTokenizer.from_pretrained(vocab_file)
        self.eod_id = self.tokenizer._convert_token_to_id("</s>")    # set eos_id to eod_id
        self.pad_id = None
