...
def get_state(
    ...
    new_key = f"sequential.{layer_idx}.{key}"
    
    res = [state_dict["module"][new_key] for state_dict in state_dicts]
    return res
...
def create_config(neox_config):
    ...
    hf_config = LlamaConfig(
        ...
        rms_norm_eps=get_key(neox_config, "rms_norm_epsilon", 1e-6),
        use_cache=True,
        pad_token_id=0, 
        bos_token_id=1,
        eos_token_id=2,
...
def convert(input_checkpoint_path, loaded_config, output_checkpoint_path):
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        ...
        state_dict["input_layernorm.weight"] = sum(get_state(loaded_tp_ranks, "input_layernorm.scale", layer_i + 2)) / len(
                loaded_tp_ranks
            )
        state_dict["post_attention_layernorm.weight"] = sum(get_state(loaded_tp_ranks, "post_attention_layernorm.scale", layer_i + 2)) / len(
                loaded_tp_ranks
            )
        ...
        tmp_query_key_value = torch.cat(
                get_state(loaded_tp_ranks, "attention.query_key_value.weight", layer_i + 2), dim=0
            )
        
        num_heads = get_key(loaded_config, "num_attention_heads")
        dims_per_head = get_key(loaded_config, "hidden_size") // num_heads
        tmp_query_key_value = tmp_query_key_value.view(num_heads, 3, dims_per_head, get_key(loaded_config, "hidden_size"))

        wq, wk, wv = torch.tensor_split(tmp_query_key_value, 3, dim=1)
        wq = wq.reshape(num_heads * dims_per_head, get_key(loaded_config, "hidden_size"))
        wk = wk.reshape(num_heads * dims_per_head, get_key(loaded_config, "hidden_size"))
        wv = wv.reshape(num_heads * dims_per_head, get_key(loaded_config, "hidden_size"))
        state_dict["self_attn.q_proj.weight"] = wq
        state_dict["self_attn.k_proj.weight"] = wk
        state_dict["self_attn.v_proj.weight"] = wv
    ...
    # hf_model.final_layer_norm.load_state_dict(
    #     {
    #         "weight": (
    #             sum(
    #                 get_state(
